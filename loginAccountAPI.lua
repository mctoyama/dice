...
local LoginAccountAPI = {}

----------------------------------------------------------------------------------
function LoginAccountAPI.post()
   ...
   email = string.lower(args["email"])

   ok, err, accountId = account.login(email, args["password"], ngx.var.remote_addr)

   if( ok ) then

      cn, sn = account.getName(accountId)

      session:set("loggedin","true")
      session:set("email", email)
      session:set("name", cn.." "..sn)
